...
def job_list(request):
    ...
    # pagination
    res_per_page = 3  # Todo: make it 10
...
def job_detail(request, pk):
    ...
    candidates = job.candidate_applies.all().count()
...
def is_applied(request, pk):
    user = request.user
    job = get_object_or_404(Job, id=pk)
    applied = job.candidate_applies.filter(user=user).exists()
...
def get_candidates_applied(request, pk):
    ...
    candidates = job.candidate_applies.all()
